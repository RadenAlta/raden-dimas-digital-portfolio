...
| Audio equalizer state | Menandai audio sedang diputar; Audio | Event `play`, `pause`, `ended`; CSS bars | Ikon/teks status tetap ada; lebih sedikit bar di mobile; reduced motion status statis | Risiko status hanya visual/CPU. Uji semua event, seek, error, keyboard |
